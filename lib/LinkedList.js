...
module.exports = class LinkedList {
    ...
    pop(){
        if(this.count > 0){
            ...
            //Caso especial: retirada do ultimo elemento da lista 
            if(this.count === 1){
                removed = this.head
                this.head = null
            }
                ...
                removed = nodePos
                // O ultimo nodo passa a ser o ultimo. Para isso,
                //indicamos que não ha nada depois dele
                nodePrev.next = null
            }
            this.count--
            return removed.data
    ...
    removeAt(position){
        //Validação da faixa de posição possiveis
        if(position >= 0 && position < this.count){
    ...
    // Retorna na posição especificada, sem retirar da lista
    getAt(position){
        // Valida a faixa de posição valida
        if(position >= 0 && position < this.count) {
